...
def get_word_plus_tag_vector(vector, tag):
    tag_vector = get_tagging_vector(tag)
    return np.concatenate((vector, tag_vector), axis=0)


def _get_word_vectors_tags_from_tokens(tokens):
    words = []
    vectors = []
    tags = []
    idx = []
    for token in tokens:
        ...
        tag = token.tag_
        words.append(word)
        vector = get_clean_word_vector(word, tag)
        vectors.append(get_word_plus_tag_vector(vector, tag))
        tags.append(tag)
...
def get_words_embeddings_tags_from_sentence(sentence):
    ...
    return _get_word_vectors_tags_from_tokens(tokens)
...
def get_words_embeddings_tags_from_text(text):
    ...
    for sent in doc.sents:
        sentences.append(_get_word_vectors_tags_from_tokens(sent))
    return sentences
...
def get_data_from_sentences(sentences):
    ...
    for sentence in sentences:
        ...
        for word, tag, entity in sentence:
            ...
            vector = get_word_plus_tag_vector(word_vector, tag)
